...
import { X, User, Camera, Eye, EyeOff, Save, ChevronDown, UserPlus, ShieldCheck, AlertTriangle, CheckCircle, Trash2 } from 'lucide-react';
...
const AddStudentModal = ({ isOpen, onClose, mode = 'add', initialData = null }) => {
  ...
  useEffect(() => {
    if (mode === 'edit' && initialData) {
      setFormData({
        ...
        course: initialData.course || 'Pharm.D',
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Enter') {
        ...
        const formElements = Array.from(document.querySelectorAll('.form-control, .btn-save, .btn-cancel'));
        const activeIdx = formElements.indexOf(document.activeElement);
        if (activeIdx > -1 && activeIdx < formElements.length - 1) {
          e.preventDefault();
          formElements[activeIdx + 1].focus();
        }
      }
    };
    if (isOpen) {
  ...
  }); // Run on every render so closures capture latest state

  const calculateProfileCompletion = () => {
    const fieldsToTrack = [
      formData.studentId, formData.fullName, formData.gender, formData.dateOfBirth,
      formData.email, formData.mobileNumber, formData.course, formData.batch,
      formData.year, formData.academicYear, photoPreview
    ];
  ...
  const handleChange = (e) => {
    ...
    if (name === 'studentId' && mode === 'add') {
      updates.username = value;
    }

    setFormData(prev => ({ ...prev, ...updates }));
    validateField(name, value);
    if (name === 'studentId' && mode === 'add') validateField('username', value);
  };
  ...
  const validateField = (name, value) => {
    ...
    if (name === 'email') {
      ...
      else if (users.some(u => u.email === value && u.id !== formData.studentId)) errorMsg = 'Email already registered.';
    }
      ...
      else if (users.some(u => (u.phone === value || u.mobileNumber === value) && u.id !== formData.studentId)) errorMsg = 'Mobile Number already registered.';
    }
    else if (name === 'studentId' && mode === 'add') {
      if (!value) errorMsg = 'Roll Number is required.';
      else if (users.some(u => u.id === value)) errorMsg = 'Roll Number already exists.';
    }
  ...
  const handleSave = async () => {
    ...
    else if (users.some(u => u.email === formData.email && u.id !== formData.studentId)) newErrors.email = 'Email already registered.';
    
    if (!formData.mobileNumber) newErrors.mobileNumber = 'Mobile Number is required.';
    else if (!/^\d{10}$/.test(formData.mobileNumber)) newErrors.mobileNumber = 'Mobile Number must be 10 digits.';
    else if (users.some(u => (u.phone === formData.mobileNumber || u.mobileNumber === formData.mobileNumber) && u.id !== formData.studentId)) newErrors.mobileNumber = 'Mobile Number already registered.';

    if (mode === 'add') {
      if (!formData.studentId) newErrors.studentId = 'Roll Number is required.';
      else if (users.some(u => u.id === formData.studentId)) newErrors.studentId = 'Roll Number already exists.';
    ...
    const newUser = {
      ...
      assignedPreceptorId: mode === 'add' ? null : initialData?.assignedPreceptorId
    };
  ...
  const handleReset = () => {
    if (mode === 'add') {
      setFormData({
        studentId: 'Y26PHD0301', fullName: '', gender: 'Male', dateOfBirth: '2005-01-01', bloodGroup: 'O+',
        aadhaarNumber: '', email: '', mobileNumber: '', course: 'Pharm.D', batch: 'Y26',
        year: 'I Year', academicYear: '2026-2027', parentName: '', parentMobile: '',
        password: '', confirmPassword: '', status: 'Active', username: 'Y26PHD0301'
      });
      setPhotoPreview(null);
    } else {
      setFormData({
      ...
      <div className="preceptor-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <div className="modal-title-group">
            ...
            <div className="modal-title-text">
              <h2>{mode === 'add' ? 'Register New Student' : 'Edit Student Profile'}</h2>
              <p>{mode === 'add' ? 'Create a new student identity record' : 'Update existing student information'}</p>
            </div>
          </div>
        ...
        <div className="modal-body custom-scrollbar">
          ...
          <div className="form-fields-container">
            ...
            <div className="form-grid" style={{ marginBottom: '2rem' }}>
              <div className="form-group">
                ...
                <input 
                  ...
                  className={`form-control ${errors.studentId ? 'error' : ''} ${mode === 'edit' ? 'read-only' : ''}`}
            ...
            <h3 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--color-primary)', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', marginBottom: '1.5rem' }}>Academic Information</h3>
            <div className="form-grid" style={{ marginBottom: '2rem' }}>
              <div className="form-group">
              ...
              <div className="form-group">
                <label>Current Year <span className="required-asterisk">*</span></label>
                <div className="form-control-wrapper">
                  <select name="year" className="form-control" value={formData.year} onChange={handleChange}>
                    ...
                    <option value="VI Year">VI Year (Internship)</option>
                  </select>
                  <ChevronDown size={18} className="select-arrow" />
                </div>
              </div>
              ...
              <div className="form-group">
                ...
                <div className="form-control-wrapper">
                  <select name="academicYear" className="form-control" value={formData.academicYear} onChange={handleChange}>
            ...
            <div className="form-grid" style={{ marginBottom: '2rem' }}>
              <div className="form-group">
                ...
                <input 
                  ...
                  placeholder="student@pharmdverse.edu"
              ...
              <div className="form-group">
                ...
                <input 
                  type="tel" name="parentMobile"
                  className="form-control"
                  placeholder="9876543210" maxLength="10"
                  value={formData.parentMobile} onChange={handleChange}
                />
              </div>
            </div>
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  readOnly
                />
                <span className="field-hint">Username will be the same as Roll Number.</span>
              </div>
            ...
            <div className="summary-card">
              ...
              <div className="summary-details" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  ...
                  <span style={{ fontWeight: 600, color: 'var(--color-primary)' }}>{formData.studentId || '-'}</span>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  ...
                  <span style={{ fontWeight: 500, color: 'var(--text-primary)' }}>{formData.batch || '-'}</span>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>Current Year</span>
                  <span style={{ fontWeight: 500, color: 'var(--text-primary)' }}>{formData.year || '-'}</span>
                </div>
              ...
              <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border-color)' }}>
                <h4 style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.75rem' }}>Registration Progress</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: formData.fullName && formData.studentId && formData.dateOfBirth ? 'var(--color-success)' : 'inherit' }}>
                    {formData.fullName && formData.studentId && formData.dateOfBirth ? <CheckCircle size={14}/> : <div style={{width: 12, height: 12, border: '1px solid currentColor', borderRadius: '50%'}}></div>}
                    Personal Information
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: formData.email && formData.mobileNumber ? 'var(--color-success)' : 'inherit' }}>
                    {formData.email && formData.mobileNumber ? <CheckCircle size={14}/> : <div style={{width: 12, height: 12, border: '1px solid currentColor', borderRadius: '50%'}}></div>}
                    Contact Information
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: formData.course && formData.batch && formData.year ? 'var(--color-success)' : 'inherit' }}>
                    {formData.course && formData.batch && formData.year ? <CheckCircle size={14}/> : <div style={{width: 12, height: 12, border: '1px solid currentColor', borderRadius: '50%'}}></div>}
                    Academic Information
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: formData.parentName || formData.parentMobile ? 'var(--color-success)' : 'inherit' }}>
                    {formData.parentName || formData.parentMobile ? <CheckCircle size={14}/> : <div style={{width: 12, height: 12, border: '1px solid currentColor', borderRadius: '50%'}}></div>}
                    Guardian Information
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: mode === 'edit' || (formData.password && formData.password === formData.confirmPassword) ? 'var(--color-success)' : 'inherit' }}>
                    {mode === 'edit' || (formData.password && formData.password === formData.confirmPassword) ? <CheckCircle size={14}/> : <div style={{width: 12, height: 12, border: '1px solid currentColor', borderRadius: '50%'}}></div>}
                    Login Credentials
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: photoPreview ? 'var(--color-success)' : 'inherit' }}>
                    {photoPreview ? <CheckCircle size={14}/> : <div style={{width: 12, height: 12, border: '1px solid currentColor', borderRadius: '50%'}}></div>}
        ...
        <div className="modal-footer" style={{ position: 'sticky', bottom: 0, backgroundColor: 'var(--bg-surface)', borderTop: '1px solid var(--border-color)', padding: '1.5rem 2rem', zIndex: 10, borderRadius: '0 0 12px 12px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '24px', marginBottom: '1rem' }}>
            ...
            {saveState === 'creating' && <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--color-primary)', fontWeight: 600, fontSize: '0.9rem' }}><div className="spinner" style={{ width: '16px', height: '16px', border: '2px solid rgba(0,0,0,0.1)', borderLeftColor: 'currentColor', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div> Creating User Account...</span>}
            {saveState === 'saving' && <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--color-primary)', fontWeight: 600, fontSize: '0.9rem' }}><div className="spinner" style={{ width: '16px', height: '16px', border: '2px solid rgba(0,0,0,0.1)', borderLeftColor: 'currentColor', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div> Saving Profile...</span>}
            {saveState === 'success' && <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--color-success)', fontWeight: 600, fontSize: '0.9rem' }}><CheckCircle size={16} /> Registration Successful</span>}
          </div>
